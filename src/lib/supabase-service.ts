  ...
  try {
    const [
      ...
      supabase
        .from("donations")
        .select(
          "id, member_id, bagisci_adi, tutar, currency, amac, odeme_yontemi, tarih, created_at",
        )
        .order("created_at", { ascending: false })
        .limit(10),
      supabase
    ...
    const safeRecentDonations =
      donations && donations.length > 0
        ? donations
            .slice(0, 5)
            .map((d: Tables["donations"]["Row"]) => mapDonation(d))
        : [];
    ...
    applications?.forEach(
      (app: { yardim_turu: string | null; onaylanan_tutar: number | null }) => {
        if (app.onaylanan_tutar && app.yardim_turu) {
          ...
          aidDistributionMap.set(
            app.yardim_turu,
            current + app.onaylanan_tutar,
          );
        }
      },
    );
...
export async function uploadDocument(
  ...
  // Use browser-compatible random string generator
  const randomStr = generateRandomHex(8);
...
export async function fetchAuditLogs(options?: {
  ...
  endDate?: string;
  search?: string;
}): Promise<PaginatedResponse<import("@/types").AuditLog>> {
  const supabase = getSupabaseClient();
  ...
  if (error) {
    throw new Error(`Audit logs fetch error: ${error.message}`);
  }
  ...
  const mappedData = (data || []).map(
    (log: {
      ...
      action: log.action as import("@/types").AuditAction,
  ...
  return toPaginatedResponse(mappedData, count, page, limit);
}

export async function fetchAuditLog(
  id: string,
): Promise<import("@/types").AuditLog | null> {
  const supabase = getSupabaseClient();
  ...
  if (error) {
    ...
    throw new Error(`Audit log fetch error: ${error.message}`);
  }
  ...
  return {
    ...
    action: data.action as import("@/types").AuditAction,
    tableName: data.table_name,
    recordId: data.record_id || "",
    oldData: data.old_data as Record<string, unknown> | undefined,
    newData: data.new_data as Record<string, unknown> | undefined,
