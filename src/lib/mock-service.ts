...
function getBeneficiaries(): IhtiyacSahibi[] {
  if (!beneficiariesCache) {
    ...
    const mainPersons = beneficiariesCache.filter(
      (b) => b.tur === "ihtiyac-sahibi-kisi",
    );
    const dependents = beneficiariesCache.filter(
      (b) => b.tur === "bakmakla-yukumlu",
    );
    ...
    for (const mainPerson of mainPersons) {
      ...
      for (
        let i = 0;
        i < numDependents && dependentIndex < dependents.length;
        i++
      ) {
...
function getGelirGider(): GelirGider[] {
  if (!gelirGiderCache) {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      const gelirkategoriler: Kategori[] = [
        "bagis",
        "bagis-kumbara",
        "yardim-destek",
        "gelir-dukkan",
      ...
      const giderkategoriler: Kategori[] = [
        "personel",
        "kira",
        "fatura",
        "malzeme",
      ...
      const kategori =
        islemTuru === "gelir"
          ? gelirkategoriler[
              Math.floor(Math.random() * gelirkategoriler.length)
            ]
          : giderkategoriler[
              Math.floor(Math.random() * giderkategoriler.length)
            ];
      ...
      gelirGiderCache.push({
        ...
        tarih,
        odemeYontemi: ["nakit", "havale", "kredi-karti"][
...
export async function fetchDonation(
  ...
  return donations.find((d) => d.id === String(id)) || null;
}
...
// Fetch single member
export async function fetchMember(id: string | number): Promise<Uye | null> {
  await delay(200);
  const members = getMembers();
  return members.find((m) => m.id === String(id)) || null;
}
...
export async function fetchApplicationById(
  ...
  return applications.find((a) => a.id === String(id)) || null;
}
...
export async function updateApplicationStatus(
  id: string | number,
  durum: BasvuruDurumu,
  degerlendirmeNotuOrOnaylananTutar?: string | number,
): Promise<SosyalYardimBasvuru> {
  await delay(300);

  const applications = getApplications();
  const application = applications.find((a) => a.id === String(id));
  ...
  if (
    degerlendirmeNotuOrOnaylananTutar &&
    typeof degerlendirmeNotuOrOnaylananTutar === "string"
  ) {
    application.degerlendirmeNotu = degerlendirmeNotuOrOnaylananTutar;
  }
...
export async function fetchBeneficiaryById(
  ...
  return beneficiaries.find((b) => b.id === String(id)) || null;
}
...
export async function updateBeneficiary(
  ...
  const beneficiaries = getBeneficiaries();
  const index = beneficiaries.findIndex((b) => b.id === String(id));
...
// Delete beneficiary
export async function deleteBeneficiary(id: string | number): Promise<boolean> {
  await delay(300);
...
export async function updateDonation(
  ...
  const donations = getDonations();
  const index = donations.findIndex((d) => d.id === String(id));
...
// Delete donation
export async function deleteDonation(id: string | number): Promise<boolean> {
  await delay(300);
...
function getVezneler(): Vezne[] {
  if (!veznelerCache) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      for (let j = 0; j < 5; j++) {
        const islemTuru =
          islemTurleri[Math.floor(Math.random() * islemTurleri.length)];
...
function getVezneIslemleri(): VezneIslem[] {
  if (!vezneIslemlerCache) {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      islemTarih.setHours(
        islemTarih.getHours() - Math.floor(Math.random() * 72),
      );
...
export async function fetchVezneler(params?: {
  page?: number;
  limit?: number;
  durum?: VezneDurum;
...
export async function fetchVezneOzet(): Promise<VezneOzet> {
  ...
  const bugununIslemleri = getVezneIslemleri().filter((vi) => {
    ...
    return vi.islemZamani.toDateString() === today.toDateString();
  });
...
export async function fetchVezneIslemleri(params?: {
  page?: number;
  limit?: number;
  vezneId?: string;
...
export async function createVezneIslem(
  ...
  if (vezne) {
    ...
    vezne.bakiye =
      vezne.bakiyeTRY + vezne.bakiyeUSD * 35 + vezne.bakiyeEUR * 38;
  }
